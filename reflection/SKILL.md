...
```mermaid
flowchart TD
    A["Raw Experience Ingestion (Stream of Consciousness / Chat / .txt / .md / Voice Dumps)"] --> B["Signal De-Noising & Subconscious Intent Isolator"]
    B --> C["4-Pillar Clinical & Philosophical Diagnostic Engine"]

    subgraph DiagnosticEngine ["🧠 4-Pillar Analytical Engine"]
        C1["Pillar 1: Objective Event vs. Emotional Narrative"]
        C2["Pillar 2: Cognitive Distortions & Defense Mechanisms Radar"]
        C3["Pillar 3: Philosophical & Archetypal Resonance (Stoic / Jungian / Vedic)"]
        C4["Pillar 4: Sovereign Samanvaya Directive (Equanimity & Re-Centering)"]
    end

    C --> DiagnosticEngine
    DiagnosticEngine --> D["Structured 'Antaryami Mirror Report'"]
    D --> E["Obsidian Root Staging (%USERPROFILE%\\Obsidian\\)"]
    E --> F{"Explicit Karan Move Directive"}
    F --> G["Antaryami/Reflections/ or Antaryami/Journals/"]
```
...
```mermaid
flowchart LR
    A["Raw Experience / Cognitive Friction"] --> B["Proactive Socratic Grill-Me Inquiry"]
    B --> C["Karan's Deeper Context & Clarifications"]
    C --> D["Pre-Execution 3-Point Alignment Schema"]
    D --> E["Generate Mirror Report & Persist Invariants"]
```
