...
The authoritative management skill and runbook for maintaining **`Motobook`** (Karan Singh Verma's Windows 11 Pro workstation) in a pristine, low-latency state tailored for the core workflow (**Terminal 50%, Edge 30%, Obsidian 15%, Notes 5%**).
...
```powershell
pwsh -NoProfile -File "$env:USERPROFILE\.gemini\antigravity-cli\skills\win-janitor\scripts\janitor.ps1" audit
```
...
```powershell
pwsh -NoProfile -File "$env:USERPROFILE\.gemini\antigravity-cli\skills\win-janitor\scripts\janitor.ps1" trim
```
...
```powershell
pwsh -NoProfile -File "$env:USERPROFILE\.gemini\antigravity-cli\skills\win-janitor\scripts\janitor.ps1" purge
```
...
```powershell
sudo pwsh -NoProfile -File "$env:USERPROFILE\.gemini\antigravity-cli\skills\win-janitor\scripts\janitor.ps1" enforce-baseline
```
...
pwsh -NoProfile -File "$env:USERPROFILE\.gemini\antigravity-cli\skills\win-janitor\scripts\janitor.ps1" diagnose ram

# Investigate High CPU spikes
pwsh -NoProfile -File "$env:USERPROFILE\.gemini\antigravity-cli\skills\win-janitor\scripts\janitor.ps1" diagnose cpu

# Reverse telemetry drift after a Windows Cumulative Update
sudo pwsh -NoProfile -File "$env:USERPROFILE\.gemini\antigravity-cli\skills\win-janitor\scripts\janitor.ps1" updates
```
